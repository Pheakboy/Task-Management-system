            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <!-- Reassign Project (Admin Only) -->
                        @if(Auth::user()->isAdmin() && $users->count() > 0)
                            <div class="mb-4">
                                <label for="created_by" class="block font-medium text-sm text-gray-700">Project Owner</label>
                                ...
                                @error('created_by')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                <p class="mt-1 text-sm text-gray-500">Current owner: {{ $project->user->name }}</p>
                            </div>
                        @endif
